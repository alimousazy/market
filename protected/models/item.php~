...
class item extends CActiveRecord
{
    ...
    public function saveFile($list)
    {
        ...
        foreach($list as $key=>$value)
        {
            $dest =  $folder_name . "/" . $this->img_map[$key] . ".jpeg"; 
            move_uploaded_file($value, $dest); 
            foreach($this->img_sizes as $size_name=>$cord)
            {
                $tmp =  $folder_name . "/" . $this->img_map[$key] . "_" . $size_name . ".jpeg";
                copy($dest,  $tmp);
    ...
    public function getRandomItem()
    {
        ...
        $sql = "SELECT colm FROM (SELECT item.*, item_id as id FROM item INNER JOIN user_points ON item.user_id = user_points.user_id WHERE points > '%s' AND points < '%s' AND status = 'active' ORDER BY rand()) AS T" ;
	 ...
	 */
	public function search($user_id = 0)
	{
